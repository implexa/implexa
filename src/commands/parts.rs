...
pub async fn get_parts(
    ...
    // Lock the part manager to keep it alive but we're not using it directly
    let _part_manager = db_state.part_manager.lock().map_err(|e| e.to_string())?;
    
    // We need to use the PartManager directly since PartManagementManager doesn't have get_all_parts
    let part_db_manager = crate::database::part::PartManager::new(&db_state.connection_manager);
    
    // Get all parts
    let parts = part_db_manager.get_all_parts()
...
pub async fn get_part(
    ...
    // We need to use the PartManager directly
    let part_db_manager = crate::database::part::PartManager::new(&db_state.connection_manager);
    
    // Get the part
    let part = part_db_manager.get_part(part_id)
        .map_err(|e| e.to_string())?;
...
pub async fn create_part(
    ...
    // Use the PartManagementManager's create_part method with the proper parameters
    // Get the app's current directory to use as repo_path
    let app_data_dir = std::env::current_dir().expect("Failed to get current directory");
    
    // Call create_part with the right parameters
    let (created_part, _) = part_manager.create_part(
        part_data.category,
...
pub async fn update_part(
    ...
    // We need to use the PartManager directly for update operations
    let part_db_manager = crate::database::part::PartManager::new(&db_state.connection_manager);
    ...
    updated_part.created_date = part.created_date;
    
    // Save the updated part
    part_db_manager.update_part(&updated_part)
        .map_err(|e| e.to_string())?;
    
    // Get the updated part
    let updated_part = part_db_manager.get_part(part_id)
        .map_err(|e| e.to_string())?;
...
pub async fn delete_part(
    ...
    // We need to use the PartManager directly for delete operations
    let part_db_manager = crate::database::part::PartManager::new(&db_state.connection_manager);
    
    // Delete the part
    part_db_manager.delete_part(part_id)
        .map_err(|e| e.to_string())?;
...
pub fn init_database_state() -> DatabaseState {
    ...
    use crate::database::part_management::{User, UserRole};
